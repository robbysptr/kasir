...
<table id="dataTableBuilder" class="table table-striped table-bordered table-hover display select tg"
    ...
    @foreach($penjualan as $i => $value)
        <tr>
            ...
            <td style="width: 13%; text-align: right;" class="tg-rv4w">{{ number_format($value->total_bayar, 0, ',', '.') }}</td>
            <td style="width: 13%; text-align: right;" class="tg-rv4w">{{ number_format($value->keuntungan(), 0, ',', '.') }}</td>
